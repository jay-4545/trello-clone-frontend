...
export default function NotificationsList({ showHeader = true, className }: NotificationsListProps) {
            ...
            {showHeader && (
                <div className="flex items-center gap-3 flex-wrap mb-4">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                        <div className="flex items-center justify-center h-9 w-9 rounded-lg bg-violet-50 text-violet-600 shrink-0">
...
function NotificationRow({
    ...
    if (n.entityType === "workspace") {
        href = `/workspaces/${n.entityId}`;
    } else if (n.entityType === "board" && meta.workspaceId) {
        href = `/workspaces/${meta.workspaceId}/boards/${n.entityId}`;
    } else if (meta.workspaceId && meta.boardId) {
        const cardId = meta.cardId ?? (n.entityType === "card" ? n.entityId : undefined);
        if (cardId) {
            href = `/workspaces/${meta.workspaceId}/boards/${meta.boardId}?card=${cardId}`;
        }
    }
    ...
    const content = (
        <>
            ...
            <div className="flex items-center gap-1 shrink-0 self-start opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                {!n.isRead && (
                    <button
